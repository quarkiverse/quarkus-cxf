 ...
 */
package io.quarkiverse.cxf.test.keystore.generator;
...
public class GenerateKeystores {
    ...
    private static X509Certificate createCertificate(
            SelfSignedX509CertificateAndSigningKey issuerSelfSignedX509CertificateAndSigningKey, PublicKey publicKey,
            String subjectDN, String serialNumber) throws Exception {
    ...
    private static KeyStore createSelfSignedKeyStore(
            SelfSignedX509CertificateAndSigningKey selfSignedX509CertificateAndSigningKey, String alias) throws Exception {
        KeyStore keyStore = loadKeyStore();

        X509Certificate certificate = selfSignedX509CertificateAndSigningKey.getSelfSignedCertificate();
        keyStore.setKeyEntry(alias, selfSignedX509CertificateAndSigningKey.getSigningKey(), GENERATED_KEYSTORE_PASSWORD,
                new X509Certificate[] { certificate });

        return keyStore;
    }

    private static KeyStore createChainedKeyStore(SelfSignedX509CertificateAndSigningKey selfSignedX509CertificateAndSigningKey,
            X509Certificate subjectCertificate, PrivateKey signingKey, String alias) throws Exception {
        KeyStore keyStore = loadKeyStore();

        X509Certificate issuerCertificate = selfSignedX509CertificateAndSigningKey.getSelfSignedCertificate();
        keyStore.setKeyEntry(alias, signingKey, GENERATED_KEYSTORE_PASSWORD,
                new X509Certificate[] { subjectCertificate, issuerCertificate });
    ...
    private static void setUpKeyStores(Path buildDir) throws Exception {
        ...
        SelfSignedX509CertificateAndSigningKey johnSelfSignedX509CertificateAndSigningKey = createBasicSelfSigned(JOHN_DN,
                SHA_256_RSA);
        SelfSignedX509CertificateAndSigningKey maxSelfSignedX509CertificateAndSigningKey = createBasicSelfSigned(MAX_DN,
                SHA_1_RSA);

        X509Certificate aliceCertificate = createCertificate(cxfcaSelfSignedX509CertificateAndSigningKey, alicePublicKey,
                ALICE_DN, ALICE_SERIAL_NUMBER);
        X509Certificate bobCertificate = createCertificate(cxfcaSelfSignedX509CertificateAndSigningKey, bobPublicKey, BOB_DN,
                BOB_SERIAL_NUMBER);

        KeyStore aliceKeyStore = createChainedKeyStore(cxfcaSelfSignedX509CertificateAndSigningKey, aliceCertificate,
                aliceSigningKey, ALICE_ALIAS);
        aliceKeyStore.setCertificateEntry(BOB_ALIAS, bobCertificate);

        KeyStore bobKeyStore = createChainedKeyStore(cxfcaSelfSignedX509CertificateAndSigningKey, bobCertificate, bobSigningKey,
                BOB_ALIAS);
        ...
        final File ALICE_JKS_FILE = new File(buildDir.toFile(), "alice.jks");
        final File BOB_JKS_FILE = new File(buildDir.toFile(), "bob.jks");
        final File JOHN_JKS_FILE = new File(buildDir.toFile(), "john.jks");
        final File MAX_JKS_FILE = new File(buildDir.toFile(), "max.jks");
    ...
    public static void main(String[] args) throws Exception {
        setUpKeyStores(Paths.get(args[0]));
    }
}
