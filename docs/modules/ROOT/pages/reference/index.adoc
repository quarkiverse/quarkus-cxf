...
Look up which {quarkus-cxf-project-name} extension provides a capability, its support level and the standards it implements, which Apache CXF modules, front ends, data bindings, transports, and annotations are supported, and how to select SOAP 1.1, SOAP 1.2, and MTOM.
...
| xref:reference/extensions/quarkus-cxf-xjc-plugins.adoc[Quarkus CXF XJC plugins from Apache CXF project for wsdl2java] +
`quarkus-cxf-xjc-plugins`
|Stable
|1.5.11
|

| xref:reference/extensions/quarkus-cxf-jaxb-plugins.adoc[Quarkus CXF XJC plugins from org.jvnet.jaxb:jaxb-plugins for wsdl2java] +
...
* `wsdl2Java` - see the xref:user-guide/contract-first-code-first/generate-java-from-wsdl.adoc[Generate the Model classes from WSDL] section of User guide
...
a| https://cxf.apache.org/dosgi-architecture.html[DOSGI]
...
a| https://cxf.apache.org/docs/websocket.html[WebSocket transport]
...
a| https://cxf.apache.org/docs/sdo.html[SDO databinding]
