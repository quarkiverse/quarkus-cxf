...
that we had no tests at all for injecting `WebServiceContext` using the `@Resource` annotation.
...
`reference/extensions/quarkus-cxf.adoc#quarkus-cxf_quarkus-cxf-codegen-wsdl2java-package-names[quarkus.cxf.codegen.wsdl2java.package-names]`
...
a `NullPointerException` without any meaningful message was thrown,
when a class from that dependency was looked up in the index.
...
Check https://quarkus.io/guides/cdi-reference#how-to-generate-a-jandex-index[How to Generate a Jandex Index] section of
Quarkus Contexts and dependency injection guide for more details.

=== Documentation improvements

* https://github.com/quarkiverse/quarkus-cxf/issues/1424[#1424] Replaced references to the non-exitent `quarkus.cxf.logging.enabled` option for the correct `quarkus.cxf.logging.enabled-for` option.
