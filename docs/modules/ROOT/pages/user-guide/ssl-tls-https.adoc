...
.historical note
====
The `quarkus.cxf.client."client-name".trust-store*` and `quarkus.cxf.client."client-name".key-store*` options were removed in 4.0.0.
Configure the stores through `quarkus.tls."tls-bucket-name".*` and select the named configuration with
