...
Read the {link-quarkus-docs-prerequisites}[Prerequisites section] of Quarkus getting started guide.
...
* GraalVM with the `native-image` command installed and the `GRAALVM_HOME` environment variable set.
  See {link-quarkus-docs-building-native-image}[Building a native executable] section of the Quarkus
  documentation.
...
* Click the *Generate your application* button to download a basic skeleton project.
