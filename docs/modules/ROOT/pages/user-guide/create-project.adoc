...
<project ...>
  ...
  <properties>
    ...
    <quarkus.platform.version><!-- Check the latest {link-boms-maven-repo-base}/{quarkus-platform-groupId-path}/quarkus-cxf-bom/ --></quarkus.platform.version>
  </properties>
...
You should always take care to import the same version of `{quarkus-platform-groupId}:quarkus-bom` and `{quarkus-platform-groupId}:quarkus-cxf-bom` into your project.
...
[NOTE]
.Quarkus version check
====
Since {quarkus-cxf-project-name} 3.39.0, the build fails if the Quarkus version used by the application
comes from a different `major.minor` stream than the Quarkus version {quarkus-cxf-project-name} was built with.
This typically indicates broken dependency management, which you can fix as described above.
The check can be relaxed via the
xref:reference/extensions/quarkus-cxf.adoc#quarkus-cxf_quarkus-cxf-on-quarkus-version-mismatch[`quarkus.cxf.on-quarkus-version-mismatch`]
