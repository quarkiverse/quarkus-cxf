...
If you use Maven, the https://cxf.apache.org/docs/maven-cxf-codegen-plugin-wsdl-to-java.html[cxf-codegen-plugin] provided by CXF
might be the handiest option.
There is also https://cxf.apache.org/docs/wsdl-to-java.html[wsdl2java] utility likewise provided by CXF.
...
We need to invoke the `wsdl2java` goal of `cxf-codegen-plugin` to generate the model classes from the WSDL:

.pom.xml
[source,xml]
----
include::example$calculator-client/pom.xml[tag=first-soap-client.adoc-cxf-codegen-plugin]
----

<1> We need to store a copy of the WSDL locally.

This will generate the model classes in `target/generated-sources` directory
where they will be automatically picked by the compiler plugin
so we are free to refer to them from our application code.
