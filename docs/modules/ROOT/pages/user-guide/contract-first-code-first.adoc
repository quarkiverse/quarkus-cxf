...
Decide whether the WSDL contract or your Java code is the source of truth: contract-first development generates Java from the WSDL with `wsdl2java`, and code-first development writes JAX-WS classes and derives the WSDL from them. Both are fully supported by {quarkus-cxf-project-name}.
...
2. xref:user-guide/contract-first-code-first/generate-wsdl-from-java.adoc[Generate the WSDL document from Java] classes at build time
