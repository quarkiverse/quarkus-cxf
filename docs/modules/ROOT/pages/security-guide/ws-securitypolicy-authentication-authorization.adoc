...
You can enforce authentication through WS-SecurityPolicy, instead of xref:user-guide/ssl-tls-https.adoc#mtls[Mutual TLS] and Basic HTTP authentication for xref:user-guide/authentication-authorization.adoc#client-http-basic-authentication[clients]
and xref:user-guide/authentication-authorization.adoc#securing-service-endpoints[services].
