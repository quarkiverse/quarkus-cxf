...
You can enforce authentication through WS-SecurityPolicy, instead of xref:user-guide/ssl.adoc#mtls[Mutual TLS] and Basic HTTP authentication for xref:user-guide/auth.adoc#_client_http_basic_authentication[clients]
and xref:user-guide/auth.adoc#_securing_service_endpoints[services].
